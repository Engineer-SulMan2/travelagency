...
export default async function AgentPackagesPage() {
  ...
  const agency = user?.agencyId
    ...
    : null;
  const rows = await getPackageCatalog(user?.agencyId, "HOLIDAY");

  return (
    <div>
      <h1 className="mb-1 text-lg font-semibold text-slate-900">Holiday packages</h1>
      <p className="mb-6 text-sm text-slate-500">Curated packages — book directly for a customer.</p>
