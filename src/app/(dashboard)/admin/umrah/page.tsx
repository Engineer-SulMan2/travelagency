...
export default async function AdminUmrahPage() {
  ...
  const agency = user?.agencyId
    ...
    : null;
  const rows = await getPackageCatalog(user?.agencyId, "UMRAH");

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="mb-1 text-lg font-semibold text-slate-900">Umrah Bookings</h1>
          <p className="text-sm text-slate-500">Curated Umrah packages — book directly for a customer.</p>
        </div>
        <Link
