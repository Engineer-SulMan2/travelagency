...
export default async function AdminUpcomingPage() {
  ...
  const items = await getUpcomingTravel({ agencyId: admin.agencyId });

  return (
    <div>
      <h1 className="mb-1 text-lg font-semibold text-slate-900">Upcoming Travel</h1>
      <p className="mb-6 text-sm text-slate-500">Every customer's travel date across your agency in the next 60 days.</p>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{item.travelerName} · {item.bookingRef} · booked by {item.bookedBy}</p>
                </div>
              </div>
