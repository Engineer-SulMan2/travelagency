...
import { getCustomerNotes, addCustomerNote, deleteCustomerNote, type CustomerNoteRow } from "@/lib/actions/customer-notes";
...
export function CustomerNotesModal({
  ...
  useEffect(() => {
    getCustomerNotes(customerId).then((n) => {
      setNotes(n);
      setLoading(false);
    });
  }, [customerId]);
  ...
  async function handleAdd() {
    ...
    setSending(true);
    const res = await addCustomerNote(customerId, draft);
    setSending(false);
    if (!res.error) {
      setDraft("");
      const fresh = await getCustomerNotes(customerId);
      setNotes(fresh);
    }
  }
      ...
      <div className="flex max-h-[80vh] w-full max-w-md flex-col rounded-2xl border border-slate-100 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">Communication log</p>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto px-5 py-4">
          {loading ? (
            ...
            <p className="py-6 text-center text-sm text-slate-400">
              No notes yet — log calls, WhatsApp replies, or anything else worth remembering.
            </p>
          ) : (
            notes.map((n) => (
        ...
        <div className="border-t border-slate-100 p-4">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={2}
              placeholder="Called — discussed price, will decide by Friday..."
